-- CRM HOTELES V3.8 COMPLETO MULTI-HOTEL CON IFRAME DINÁMICO
...
DROP TABLE IF EXISTS `clientes_no_gratos`;
...
CREATE TABLE `habitaciones_bloqueos` (
  ...
  CONSTRAINT `fk_bloqueos_hotel` FOREIGN KEY (`id_hotel`) REFERENCES `hoteles` (`id_hotel`) ON UPDATE CASCADE ON DELETE CASCADE,
  CONSTRAINT `fk_bloqueos_habitacion` FOREIGN KEY (`id_habitacion`) REFERENCES `habitaciones` (`id_habitacion`) ON UPDATE CASCADE ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE `clientes_no_gratos` (
  ...
  CONSTRAINT `fk_no_gratos_hotel` FOREIGN KEY (`id_hotel`) REFERENCES `hoteles` (`id_hotel`) ON UPDATE CASCADE ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE `reservas` (
  ...
  `id_usuario_tomada` int DEFAULT NULL,
  ...
  KEY `idx_reservas_usuario_tomada` (`id_usuario_tomada`),
  KEY `idx_reservas_bloqueo_hasta` (`bloqueo_hasta`),
  ...
  CONSTRAINT `fk_reservas_usuario_tomada` FOREIGN KEY (`id_usuario_tomada`) REFERENCES `usuarios` (`id_usuario`) ON UPDATE CASCADE ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
INSERT INTO `reservas` (`id_hotel`,`id_categoria`,`id_habitacion`,`fecha_checkin`,`fecha_checkout`,`precio_final`,`estado_reserva`,`canal_reserva`,`notas`) VALUES
(1,2,NULL,DATE_ADD(CURDATE(), INTERVAL 5 DAY),DATE_ADD(CURDATE(), INTERVAL 7 DAY),360,'Pendiente','Web','Consulta inicial desde web'),
(1,3,3,DATE_ADD(CURDATE(), INTERVAL 10 DAY),DATE_ADD(CURDATE(), INTERVAL 12 DAY),440,'Confirmada','Consulta WhatsApp','Cliente confirmó por WhatsApp'),
(2,7,NULL,DATE_ADD(CURDATE(), INTERVAL 4 DAY),DATE_ADD(CURDATE(), INTERVAL 6 DAY),380,'Pendiente','Web','Consulta Inkarian'),
(3,12,NULL,DATE_ADD(CURDATE(), INTERVAL 3 DAY),DATE_ADD(CURDATE(), INTERVAL 5 DAY),340,'Pendiente','Web','Consulta Flora');
